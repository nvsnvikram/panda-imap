...
#define SSLCIPHERLIST "ALL:!aNULL:!eNULL:!SSLv2:!ADH:!EXP:!LOW:!MD5:!RC4:!3DES:!IDEA"
...
static SSLSTREAM*
ssl_start(TCPSTREAM *tstream, char *host, unsigned long flags) {
     char *reason, tmp[MAILTMPLEN];
     sslfailure_t sf = (sslfailure_t) mail_parameters(NIL, GET_SSLFAILURE, NIL);
     blocknotify_t bn = (blocknotify_t) mail_parameters(NIL,
                                                        GET_BLOCKNOTIFY,
                                                        NIL);
     void *data = (*bn) (BLOCK_SENSITIVE,NIL);
     SSLSTREAM *stream = (SSLSTREAM *) memset(fs_get(sizeof(SSLSTREAM)),
                                              0,
                                              sizeof(SSLSTREAM));
     ...
     if (reason) {			/* failed? */
          ...
          case '*':			/* certificate failure */
               ...
               if (sf)
                    (*sf)(host, reason, flags);
               else { /* no error callback, build error message */
                    sprintf(tmp,
                            "Certificate failure for %.80s: %.512s",
                            host,
                            reason);
                    mm_log(tmp, ERROR);
               }
...
static char*
ssl_start_work(SSLSTREAM *stream, char *host, unsigned long flags) {
     BIO *bio;
     X509 *cert;
     unsigned long sl, tl;
     char *s, *t, *err, tmp[MAILTMPLEN];
     sslcertificatequery_t scq =
          (sslcertificatequery_t) mail_parameters(NIL,
                                                  GET_SSLCERTIFICATEQUERY,
                                                  NIL);
     sslclientcert_t scc =
          (sslclientcert_t) mail_parameters(NIL,
                                            GET_SSLCLIENTCERT,
                                            NIL);
     sslclientkey_t sck =
          (sslclientkey_t) mail_parameters(NIL,
                                           GET_SSLCLIENTKEY,
                                           NIL);
     ...
     if (flags & NET_NOVALIDATECERT)
          SSL_CTX_set_verify(stream->context,
                             SSL_VERIFY_NONE,
                             NIL);
     else
          SSL_CTX_set_verify(stream->context,
     ...
     if (scc && (s = (*scc) ()) && (sl = strlen (s))) {
          if (cert = PEM_read_bio_X509(bio = BIO_new_mem_buf(s, sl),
                                       NIL,
                                       NIL,
                                       NIL)) {
          ...
          if (s != t)
               memset(s, 0, sl);/* erase certificate if different from key */
     }
     ...
     if (SSL_in_init(stream->con))
          SSL_total_renegotiations(stream->con);
     /* now negotiate SSL */
...
static int
ssl_open_verify (int ok, X509_STORE_CTX *ctx) {
     ...
     if (!ok) {			/* in case failure */
          err = (char *)X509_verify_cert_error_string(X509_STORE_CTX_get_error(ctx));
          X509_NAME_oneline(X509_get_subject_name(X509_STORE_CTX_get_current_cert (ctx)), cert, 255);
          ...
          else if ((*scq) (err, ssl_last_host, cert))
               ok = T;
...
ssl_server_init(char *server) {
     ...
     if (stat(cert, &sbuf))
          sprintf(cert, "%s/%s.pem", SSL_CERT_DIRECTORY, server);
     ...
     if (!(stream->context = SSL_CTX_new(TLS_server_method())))
          ...
          /* load certificate */
               ...
               syslog(LOG_ALERT,
                      ...
                      tcp_clienthost());
          /* load key */
          else if (!(SSL_CTX_use_RSAPrivateKey_file(stream->context,
                                                    key,
                                                    SSL_FILETYPE_PEM)))
               syslog(LOG_ALERT,
               ...
               if (!(stream->con = SSL_new(stream->context)))
                    ...
                    if (SSL_accept(stream->con) < 0)
                         ...
                         sslstdio = (SSLSTDIOSTREAM *)
                              memset(fs_get(sizeof(SSLSTDIOSTREAM)),
                                     0,
                                     sizeof(SSLSTDIOSTREAM));
...
static RSA*
ssl_genkey (SSL *con, int export, int keylength) {
     unsigned long i;
     static RSA *key = NIL;
     if (!key) {
          BIGNUM *e;
          if (!BN_dec2bn(&e, "65537")) {
               syslog(LOG_ALERT,
                      "Unable to create exponent, host=%.80s",
                      tcp_clienthost());
               exit(1);
                      
          }
          if (!RSA_generate_key_ex(key, 1024, e, NIL)) {
               syslog(LOG_ALERT,
